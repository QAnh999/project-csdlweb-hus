...
const IndexPage = () => {
  ...
  const handleCheckin = async () => {
    if (!checkinCode) { alert("Vui lòng nhập đầy đủ thông tin"); return; }
    const code = checkinCode.trim().toUpperCase();
    const token = JSON.parse(localStorage.getItem("auth") || "{}")?.access_token;
    if (!token) { alert("Chưa đăng nhập"); return; }

    try {
      const response = await fetch(`http://localhost:8000/checkin/online?reservation_code=${code}`, {
        headers: { "Authorization": `Bearer ${token}` }
      });
      if (!response.ok) {
        const errData = await response.json().catch(() => ({}));
        alert(errData.detail || "Mã đặt chỗ không hợp lệ");
        return;
      }
      ...
      // Nếu OK, fallback local nếu cần, rồi navigate
      const bookingStr = localStorage.getItem(code); // Optional legacy
  ...
  const handleSearchBooking = async () => {
    ...
    try {
      ...
      const historyResponse = await fetch("http://localhost:8000/booking/history", {
        headers: { "Authorization": `Bearer ${token}` }
      });
