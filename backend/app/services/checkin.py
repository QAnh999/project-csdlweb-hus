...
from utils.generator import generate_boarding_pass_code, generate_qr_base64
...
class CheckinService:
    ...
    def get_checkin_info(self, db: Session, reservation_code: str, flight_id: int):
        try: 
            details = self.repo.get_for_checkin(db, reservation_code, flight_id)
            if not details:
                raise ValueError("Không tìm thấy thông tin hành khách")
    ...
    def display_ticket(self, db: Session, reservation_detail_id: int):
        try:
            detail = self.repo.get(db, reservation_detail_id)
            if not detail:
                raise ValueError("Không tìm thấy chi tiết đặt chỗ")
            if detail.checkin_status != "checked_in":
                raise ValueError("Chưa làm thủ tục, không thể lấy vé")
            
            ticket = ticket_repository.get_by_reservation_detail_id(db, reservation_detail_id)
            if not ticket:
                raise ValueError("Không tìm thấy vé")
            
            return {
                "boarding_pass_code": ticket.ticket_number,
